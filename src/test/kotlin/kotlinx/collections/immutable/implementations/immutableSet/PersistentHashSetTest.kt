...
class PersistentHashSetTest {
    ...
    @Test
    fun collisionTests() {
        var set = persistentHashSetOf<ElementWrapper<Int>>()

        assertTrue(set.add(ElementWrapper(1, 1)).contains(ElementWrapper(1, 1)))
        ...
        repeat(times = elementsToAdd) { index ->
            ...
            if (!set.contains(wrapper(index))) {
                ...
                for (wrapper in collisions) {
                    ...
                    val sameSet = set.remove(ElementWrapper(wrapper.element, Int.MIN_VALUE))
    ...
    fun randomOperationsTests() {
        repeat(times = 1) {
            ...
            val random = Random()
            val mutableSets = List(10) { mutableSetOf<ElementWrapper<Int>?>() }
            val immutableSets = MutableList(10) { persistentHashSetOf<ElementWrapper<Int>?>() }
            ...
            repeat(times = operationCount) {
                ...
                val element = if (random.nextDouble() < 0.001) {
                    ...
                    ElementWrapper(random.nextInt(), hashCodes[hashCodeIndex])
                }
